  ...
  <div v-if="completeDirectory !== null">
    ...
    >
      <input
        ...
        @change="handleChange"
      />
      <label v-if="!sending" class="label" :for="inputFileId">
        <span><i class="fa-doc-add"></i>{{ $t("add") }}</span>
      </label>
      <div v-else-if="progressFilename">
        <div class="progress-value" :style="{ width: progressNumber + '%' }"></div>
        <div class="filename">
          {{ progressFilename }}
        </div>
        <div class="counter ref">{{ uploadedFiles + 1 }} / {{ filesToUpload }}</div>
      </div>
    </div>
      ...
      return new Promise((resolve) => {
        ...
        request.upload.onprogress = function (event) {
          if (event.lengthComputable) {
            that.progressNumber = Math.round((event.loaded * 100) / event.total);
          }
        };
